...
function useFinanceChat() {
  ...
  useEffect(() => {
    const getInitialAdvice = async () => {
      ...
      try {
        const token = document.cookie.split("auth=")[1];
        const response = await fetch("http://localhost:5000/api/chat/initial", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        setMessages([
          {
            id: Date.now().toString(),
            role: "assistant",
            content: "Error loading initial advice. Please try again later.",
          },
        ]);
      } finally {
        setIsLoading(false);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const txResponse = await fetch(
        "http://localhost:5000/api/dashboard/transactions",
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            {transactions.length === 0 && (
              <Card className="p-6">
                ...
                <p>
                  You don't have any transactions right now, but you can upload
                  a JSON file with your transactions.
                </p>
